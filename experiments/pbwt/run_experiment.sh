...
do
	...
	# naive
	/usr/bin/time -f"$usrbintimeformat" -o "$tmpfile" "$mincard_naive" msa.fa --disable-perfect-segments -U $U -o mincard_naive_U${U}.eds
	t1=$(<"$tmpfile")
	naive_times+=("$t1")
	# recursive
	/usr/bin/time -f"$usrbintimeformat" -o "$tmpfile" "$mincard_recursive" msa.fa --disable-perfect-segments -U $U -o mincard_recursive_U${U}.eds
	t2=$(<"$tmpfile")
	recursive_times+=("$t2")
	# rmq
	/usr/bin/time -f"$usrbintimeformat" -o "$tmpfile" "$mincard_rmq" msa.fa --disable-perfect-segments -U $U -o mincard_rmq_U${U}.eds
	t3=$(<"$tmpfile")
	rmq_times+=("$t3")
